...
module.exports = class Cli {
  ...
  argv() {
    // let argvCache;
    // if (this.argvCache) {
    //   argvCache = this.argvCache
    // } else {
    //   argvCache = this.argvCache = require('yargs').help(false).version(false).completion(
    //         'completion',
    //         'For use by shell autocompletion facilities',
    //         (current, argv) => {
    //             const Lando = require('./../lib/lando');
    //             const lando = new Lando();//this.defaultConfig());
    //             return lando.bootstrap('tasks')
    //               .then(lando => {
    //                 return lando.autocomplete.getCompletions(current);
    ...
    // }
    // return argvCache;
    let compIndex = _.findIndex(process.argv, value => value === '--get-yargs-completions');
    if (compIndex !== -1) {
      process.argv[compIndex] = 'get-completions';
    }
  ...
  init(yargs, tasks, config, userConfig) {
    ...
    // now that tasks are initialized, add yarg completion
    let args = _.clone(yargs.argv._);
    let compIndex = _.findIndex(args, value => value === 'get-completions');
    if (compIndex !== -1) {
      args[compIndex] = '--get-yargs-completions';
      args['get-yargs-completions'] = yargs.argv._[compIndex + 1];
      args.getYargsCompletions = yargs.argv._[compIndex + 1];
    }
    yargs.getCompletion(args, candidates => {
      _.forEach( candidates, candidate => console.log(candidate) );
    });
    // yargs.completion();
